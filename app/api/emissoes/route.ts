...
export async function GET() {
  try {
    // Busca empresas que são candidatas a emissão
    // (Tem certificado OU já emitiram nota)
    const emissores = await prisma.empresa.findMany({
      where: {
        OR: [
            { certificadoA1: { not: null } }, // Tem certificado
            { notasEmitidas: { some: {} } }   // Já emitiu nota
        ]
      },
      include: {
        _count: {
    ...
    // Calcula erros recentes (gambiarra performática para o MVP)
