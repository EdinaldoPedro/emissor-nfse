...
export default function DetalheTicketCliente() {
  ...
  useEffect(() => { 
      carregar(); 
      const interval = setInterval(carregar, 5000); // Polling mais rápido (5s)
  ...
  const enviarMsg = async () => {
      ...
      await fetch('/api/suporte/tickets/mensagem', {
          ...
          body: JSON.stringify({ 
              ticketId: id, 
              mensagem: novaMsg 
              // Anexos podem ser adicionados aqui seguindo o padrão do admin
          })
      });
      setNovaMsg('');
      carregar();
    ...
    <div className="flex flex-col h-screen bg-slate-100">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
            {ticket.mensagens.map((msg: any) => {
                ...
                // Se o usuário da msg tem cargo, é Staff. Senão, é o cliente.
                const isStaff = ['ADMIN','SUPORTE','MASTER', 'SUPORTE_TI'].includes(msg.usuario.role);
