...
export default function NovoChamado() {
  ...
  // Carrega opções do catálogo do banco
  useEffect(() => {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (file) {
          ...
          reader.onloadend = () => {
              setForm(prev => ({
                  ...prev,
                  anexoBase64: reader.result as string,
                  anexoNome: file.name
              }));
          };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
      ...
      // O Backend vai definir a prioridade automaticamente baseado no ID do assunto
      ...
      else alert("Erro ao criar chamado. Tente novamente.");
      setLoading(false);
  };
    ...
    <div className="min-h-screen bg-slate-50 p-6 flex justify-center">
        <div className="bg-white p-8 rounded-xl shadow-md w-full max-w-2xl h-fit">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                {/* SELEÇÃO DO ASSUNTO (CATÁLOGO) */}
                <div>
                    <label className="block text-sm font-bold text-slate-700 mb-1">Qual o motivo do contato?</label>
                    <select 
                        required 
                        className="w-full p-3 border rounded-lg bg-white outline-blue-500 text-slate-700 focus:ring-2 focus:ring-blue-100 transition" 
                        value={form.assuntoId} 
                        onChange={e => setForm({...form, assuntoId: e.target.value})}
                    >
                    ...
                    {catalogo.length === 0 && <p className="text-xs text-red-500 mt-1">Carregando assuntos...</p>}
                </div>
                ...
                <div>
                    ...
                    <textarea 
                        required 
                        rows={6} 
                        className="w-full p-3 border rounded-lg outline-blue-500 focus:ring-2 focus:ring-blue-100 transition" 
                        placeholder="Descreva o que aconteceu, inclua número da nota ou mensagem de erro se houver..."
                ...
                <div>
                    ...
                    {!form.anexoBase64 ? (
                        <label className="flex items-center gap-2 w-fit px-4 py-2 border border-dashed border-blue-300 bg-blue-50 text-blue-700 rounded-lg cursor-pointer hover:bg-blue-100 transition">
                            <Paperclip size={18}/>
                            <span className="text-sm font-bold">Escolher arquivo (Máx 10MB)</span>
                            <input type="file" className="hidden" onChange={handleFileChange} accept="image/*,.pdf"/>
                        </label>
                    ) : (
                        <div className="flex items-center gap-3 bg-slate-100 p-3 rounded-lg w-fit border border-slate-200">
                            <FileText size={20} className="text-slate-500"/>
                            <span className="text-sm font-medium max-w-[200px] truncate text-slate-700">{form.anexoNome}</span>
                            <button type="button" onClick={removeAnexo} className="text-red-500 hover:bg-red-100 p-1 rounded transition">
                                <X size={16}/>
                            </button>
                        </div>
                    )}
                ...
                <button disabled={loading} className="w-full bg-blue-600 text-white py-3 rounded-lg font-bold hover:bg-blue-700 flex justify-center gap-2 items-center shadow-lg shadow-blue-100 transition disabled:opacity-70">
